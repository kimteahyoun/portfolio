...
const MyBuyList = ({ location, history }) => {
  ...
  const callSellList = async () => {
    const result = await axios.get(`/api/tradeinfo/buylist?seller=${seller}&page=${page}`)
            ...
            <>
              <tr key={list.paycode}>
                ...
                {(list.sellercondition === 0) && (list.seller !== list.buyer) ?
                  ...
                  <td><Button onClick={() => history.push(`/my/review/insert/${list.paycode}?buyer=${list.seller}&seller=${list.buyer}`)}>
                    후기쓰러가기</Button></td>
                  :
                  <td>후기 작성 완료</td>}
              </tr>
