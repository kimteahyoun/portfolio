...
const HeaderPage = ({ history }) => {
  ...
  const callUserRead = async() => {
    //로그인한 사용자 정보를 읽는다. const result = await axios.get(`/api/user/read/${uid}`); setUser(result.data); //읽지 않은 메시지 수를 구한다. const readDate = !result.data.notice_read_date ? '' : result.data.notice_read_date; //console.log('readDate:', readDate); const q=query(collection(db, 'messages'), where('date', '>', readDate)); onSnapshot(q, (snapshot)=> {
