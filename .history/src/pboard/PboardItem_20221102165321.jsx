...
const PboardItem = ({ postList }) => {
  ...
  const onToggleLike = async (pwriter, pcode) => {
    ...
    console.log(lovecondition);
    console.log(loveList);
    lovecondition === true ? await axios.post(`/api/pboard/dislove`, formData)
        ...
        <ButtonGroup>
          {sessionStorage.getItem('uid') &&
            <Button onClick={() => onToggleLike(unickname, pcode)} variant="primary">
              <img src={lovecondition ? "../image/heart.png" : "../image/emptyheart.png"} width={30} />
              {plike}
            </Button>
          }
