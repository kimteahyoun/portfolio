...
const PboardItem = ({ postList }) => {
  ...
  const [loveList, setLoveList] = useState(false);
  ...
  const onToggleLike = async (pwriter, pcode) => {
    ...
    if(!loveList) await axios.post(`/api/pboard/love`,formData)
    else{
      loveList.lovecondition===false ?  await axios.post(`/api/pboard/love`, formData) 
      : await axios.post(`/api/pboard/dislove`, formData)
    }
        ...
        <ButtonGroup>
          {sessionStorage.getItem('uid') &&
            <Button onClick={() => onToggleLike(loginUser.unickname, pcode)} variant="primary">
              <img src={loveList.lovecondition ? "../image/heart.png" : "https://dummyimage.com/100x100"/* "../image/emptyheart.png" */} width={30} />
              {plike}
            </Button>
          }
