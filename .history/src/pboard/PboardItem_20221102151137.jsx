...
const PboardItem = ({ postList }) => {
  ...
  const {unickname, isLike}=likeList;


  const onToggleLike = async (pwriter, pcode) => {
    const formData = new FormData();
    formData.append('pcode', pcode);
/*     formData.append('lcode',new Date().getTime() + Math.random()); */
    formData.append('unickname', /* 로그인한 유저의 unickname */);
    isLike ? await axios.post(`/api/pboard/dislike`, formData)
