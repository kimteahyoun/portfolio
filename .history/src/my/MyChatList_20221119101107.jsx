...
const MyChatList = ({ history, match }) => {
    ...
    const sendMessage = async (e) => {
        if (e.keyCode === 13) {
            ...
            //pcode ㅡ> pwriter(unickanme) ㅡ> 로그인한아이디(uid) 
            //room ㅡ> pcode ㅡ> 로그인 아이디 
            const docRef = doc(db, "rooms", pcode);
    ...
    const getMessages = () => {
        const q = query(
            collection(db, `rooms/${pcode}/${sessionStorage.getItem('uid')}`),
            orderBy('date', 'asc'),
            limit(100)
        );
