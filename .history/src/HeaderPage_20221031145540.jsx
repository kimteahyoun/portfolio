...
const HeaderPage = ({ history }) => {
  ...
  useEffect(() => {
    //로그인한 사용자가 바뀔 경우 사용자 정보를 다시 읽는다. if(uid !== null) callUserRead(); }, [uid]); useEffect(()=> {
    //알림페이지에서 떠나기 전에 read date를 수정한다. return async()=> {
    if (location.pathname === '/notice') {
      await axios.post(`/api/user/update/notice_read_date/${uid}`);
      callUserRead();
    }
  }
    }, [location]);
